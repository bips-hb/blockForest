...
library(blockForest)
context("blockForest_class")

## Initialize the random forest for classification
dat <- data.matrix(iris)

rg.class <- blockForest(Species ~ ., data = iris, verbose = FALSE, write.forest = TRUE)
rg.mat   <- blockForest(dependent.variable.name = "Species", data = dat, write.forest = TRUE, classification = TRUE)

## Basic tests (for all random forests equal)
test_that("classification result is of class blockForest with 15 elements", {
  expect_is(rg.class, "blockForest")
...
test_that("Alternative interface works for classification", {
  rf <- blockForest(dependent.variable.name = "Species", data = iris)
...
test_that("save.memory option works for classification", {
  rf <- blockForest(Species ~ ., data = iris, save.memory = TRUE)
  expect_equal(rf$treetype, "Classification")
})
...
test_that("Alternative interface classification prediction works if only independent variable given, one independent variable", {
  ...
  rf <- blockForest(dependent.variable.name = "y", data = dt, num.trees = 5, write.forest = TRUE)
  expect_silent(predict(rf, dt))
  expect_silent(predict(rf, dt[, 1, drop = FALSE]))
  
  dt2 <- data.frame(y = factor(rbinom(n, 1, 0.5)), x = runif(n))
  rf <- blockForest(dependent.variable.name = "y", data = dt2, num.trees = 5, write.forest = TRUE)
...
test_that("default splitrule is Gini for classification", {
  ...
  set.seed(42)
  rf2 <- blockForest(Species ~ ., iris, num.trees = 5, splitrule = "gini")
...
test_that("default splitrule is Gini for probability", {
  set.seed(42)
  rf1 <- blockForest(Species ~ ., iris, num.trees = 5, probability = TRUE)
  
  set.seed(42)
  rf2 <- blockForest(Species ~ ., iris, num.trees = 5, probability = TRUE, splitrule = "gini")
...
test_that("splitrule extratrees is different from Gini for classification", {
  set.seed(42)
  rf1 <- blockForest(Species ~ ., iris, num.trees = 5, splitrule = "extratrees")
...
test_that("splitrule extratrees is different from Gini for probability", {
  set.seed(42)
  rf1 <- blockForest(Species ~ ., iris, num.trees = 5, probability = TRUE, splitrule = "extratrees")
...
test_that("Working with numerically almost exact splitting values", {
  ...
  expect_silent(blockForest(a ~ ., data = dat, num.threads = 1, num.trees = 1))
})

test_that("No error if unused factor levels in outcome", {
  expect_warning(rf <- blockForest(Species ~ ., iris[1:100, ], num.trees = 5),
