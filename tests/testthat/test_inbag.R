...
library(blockForest)
context("blockForest_inbag")

## Tests
test_that("Inbag count matrix if of right size, with replacement", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE)
...
test_that("Inbag count matrix if of right size, without replacement", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, replace = FALSE, keep.inbag = TRUE)
  expect_equal(dim(data.frame(rf$inbag.counts)), 
              c(nrow(iris), rf$num.trees))
})

test_that("Inbag count matrix if of right size, with replacement, weighted", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, case.weights = runif(nrow(iris)), keep.inbag = TRUE)
  expect_equal(dim(data.frame(rf$inbag.counts)), 
              c(nrow(iris), rf$num.trees))
})

test_that("Inbag count matrix if of right size, without replacement, weighted", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, replace = FALSE, case.weights = runif(nrow(iris)), keep.inbag = TRUE)
  expect_equal(dim(data.frame(rf$inbag.counts)), 
              c(nrow(iris), rf$num.trees))
})
...
test_that("Number of samples is right sample fraction, replace=FALSE, default", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = FALSE)
...
test_that("Number of samples is right sample fraction, replace=FALSE, 0.3", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = FALSE, sample.fraction = 0.3)
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
  sample.fraction <- mean(num.inbag/nrow(iris))
...
test_that("Number of samples is right sample fraction, replace=TRUE, default", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = TRUE)
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
...
test_that("Number of samples is right sample fraction, replace=TRUE, 0.5", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = TRUE, sample.fraction = 0.5)
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
...
test_that("Number of samples is right sample fraction, replace=FALSE, 0.3, weighted", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = FALSE, sample.fraction = 0.3, case.weights = runif(nrow(iris)))
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
  sample.fraction <- mean(num.inbag/nrow(iris))
...
test_that("Number of samples is right sample fraction, replace=TRUE, 0.5, weighted", {
  rf <- blockForest(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = TRUE, sample.fraction = 0.5, case.weights = runif(nrow(iris)))
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
