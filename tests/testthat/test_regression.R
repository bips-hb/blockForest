...
library(blockForest)
context("blockForest_reg")

## Initialize the random forest for regression
rg.reg <- blockForest(Sepal.Length ~ ., data = iris, verbose = FALSE, write.forest = TRUE)

## Basic tests (for all random forests equal)
test_that("regression result is of class blockForest with 15 elements", {
  expect_is(rg.reg, "blockForest")
...
test_that("Alternative interface works for regression", {
  rf <- blockForest(dependent.variable.name = "Sepal.Length", data = iris)
  expect_equal(rf$treetype, "Regression")
})

test_that("Matrix interface works for regression", {
  rf <- blockForest(dependent.variable.name = "Sepal.Length", data = data.matrix(iris), write.forest = TRUE)
  expect_equal(rf$treetype, "Regression")
...
test_that("Matrix interface prediction works for regression", {
  ...
  rf <- blockForest(dependent.variable.name = "Sepal.Length", data = dat, write.forest = TRUE)
  expect_silent(predict(rf, dat))
})

test_that("save.memory option works for regression", {
  rf <- blockForest(Sepal.Length ~ ., data = iris, save.memory = TRUE)
  expect_equal(rf$treetype, "Regression")
})

test_that("predict.all for regression returns numeric matrix of size n x trees", {
  rf <- blockForest(Petal.Width ~ ., iris, num.trees = 5, write.forest = TRUE)
...
test_that("Variance splitting not working on classification data", {
  expect_error(blockForest(Species ~ ., iris, splitrule = "variance"))
})

## Splitrule
test_that("default splitrule is variance for regression", {
  set.seed(42)
  rf1 <- blockForest(Sepal.Length ~ ., iris, num.trees = 5)
  
  set.seed(42)
  rf2 <- blockForest(Sepal.Length ~ ., iris, num.trees = 5, splitrule = "variance")
...
test_that("splitrule extratrees is different from variance for regression", {
  set.seed(42)
  rf1 <- blockForest(Sepal.Length ~ ., iris, num.trees = 5, splitrule = "extratrees")
...
test_that("splitrule maxstat is different from variance for regression", {
  set.seed(42)
  rf1 <- blockForest(Sepal.Length ~ ., iris, num.trees = 5, splitrule = "maxstat")
