...
test_that("Same result with sparse data for iris classification", {
  set.seed(56)
  rf1 <- blockForest(data = iris_sparse, dependent.variable.name = "Species", classification = TRUE, num.trees = 5)
  
  set.seed(56)
  rf2 <- blockForest(data = iris, dependent.variable.name = "Species", num.trees = 5)
...
test_that("Same result with sparse data for iris regression", {
  set.seed(56)
  rf1 <- blockForest(data = iris_sparse, dependent.variable.name = "Sepal.Length", classification = FALSE, num.trees = 5)
  
  set.seed(56)
  rf2 <- blockForest(data = iris, dependent.variable.name = "Sepal.Length", num.trees = 5)
...
test_that("Same result with sparse data for 0/1 classification", {
  set.seed(56)
  rf1 <- blockForest(data = dat_sparse, dependent.variable.name = "y", classification = TRUE, num.trees = 5)
  
  set.seed(56)
  rf2 <- blockForest(data = dat, dependent.variable.name = "y", classification = TRUE, num.trees = 5)
...
test_that("Same result with sparse data for 0/1 regression", {
  set.seed(56)
  rf1 <- blockForest(data = dat_sparse, dependent.variable.name = "y", classification = FALSE, num.trees = 5)
  
  set.seed(56)
  rf2 <- blockForest(data = dat, dependent.variable.name = "y", num.trees = 5)
...
test_that("Prediction is the same if training or testing data is sparse", {
  ...
  set.seed(42)
  rf1 <- blockForest(data = train, dependent.variable.name = "Species", classification = TRUE, num.trees = 5)
  pred1 <- predict(rf1, test)
  pred1_sparse <- predict(rf1, test_sparse)
  
  set.seed(42)
  rf2 <- blockForest(data = train_sparse, dependent.variable.name = "Species", classification = TRUE, num.trees = 5)
