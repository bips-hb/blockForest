...
rg.prob <- blockForest(Species ~ ., data = iris.train, write.forest = TRUE, probability = TRUE)
...
test_that("growing works for single observations, probability prediction", {
  expect_warning(rf <- blockForest(Species ~ ., iris[1, ], write.forest = TRUE, probability = TRUE), 
...
test_that("save.memory option works for probability", {
  rf <- blockForest(Species ~ ., data = iris, probability = TRUE, save.memory = TRUE)
  expect_equal(rf$treetype, "Probability estimation")
})

test_that("predict works for single observations, probability prediction", {
  rf <- blockForest(Species ~ ., iris, write.forest = TRUE, probability = TRUE)
...
test_that("Probability estimation works correctly if labels are reversed", {
  ...
  ## Train
  rf <- blockForest(dependent.variable.name = "label", data = dat, probability = TRUE, 
               write.forest = TRUE, num.trees = 5)
  rf.rev <- blockForest(dependent.variable.name = "label", data = dat.rev, probability = TRUE, 
                   write.forest = TRUE, num.trees = 5)
...
test_that("No error if unused factor levels in outcome", {
  expect_warning(rf <- blockForest(Species ~ ., iris[1:100, ], num.trees = 5, probability = TRUE),
