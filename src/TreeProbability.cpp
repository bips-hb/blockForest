...
bool TreeProbability::splitNodeInternal(size_t nodeID, std::vector<std::vector<size_t>>& possible_split_varIDs) {
...
bool TreeProbability::findBestSplit(size_t nodeID, std::vector<std::vector<size_t>>& possible_split_varIDs) {
...
void TreeProbability::findBestSplitValueSmallQ(size_t nodeID, size_t varID, size_t num_classes, size_t* class_counts,
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    ...
    double decrease = block_weight * (sum_left / (double) n_left + sum_right / (double) n_right[i]);
...
void TreeProbability::findBestSplitValueLargeQ(size_t nodeID, size_t varID, size_t num_classes, size_t* class_counts,
  ...
  for (size_t i = 0; i < num_unique - 1; ++i) {
    ...
    // Decrease of impurity
    double decrease = block_weight * (sum_right / (double) n_right + sum_left / (double) n_left);
...
void TreeProbability::findBestSplitValueUnordered(size_t nodeID, size_t varID, size_t num_classes, size_t* class_counts,
  ...
  for (size_t local_splitID = 1; local_splitID < num_splits / 2; ++local_splitID) {
    ...
    // Decrease of impurity
    double decrease = block_weight * (sum_left / (double) n_left + sum_right / (double) n_right);
...
bool TreeProbability::findBestSplitExtraTrees(size_t nodeID, std::vector<std::vector<size_t>>& possible_split_varIDs) {
    ...
    for (auto& varID : possible_split_varIDs[i]) {
      ...
      if (data->isOrderedVariable(varID)) {
        ...
        findBestSplitValueExtraTreesUnordered(nodeID, varID, num_classes, class_counts, num_samples_node, best_value,
            best_varID, best_decrease, block_weight);
      }
    }
  }
