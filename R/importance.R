...
##' Extract variable importance of blockForest object.
##'
##'
##' @title blockForest variable importance
...
##'
##' @title blockForest variable importance p-values
##' @param x \code{blockForest} or \code{holdoutRF} object.
...
##' @param ... Further arguments passed to \code{blockForest()}. Used in the "altmann" method only.
...
##' rf.sim <- blockForest(y ~ ., dat, importance = "impurity_corrected")
##' importance_pvalues(rf.sim, method = "janitza")
##' 
##' ## Permutation p-values 
##' \dontrun{
##' rf.iris <- blockForest(Species ~ ., data = iris, importance = 'permutation')
##' importance_pvalues(rf.iris, method = "altmann", formula = Species ~ ., data = iris)
##' }
##' @seealso \code{\link{blockForest}}
##' @author Marvin N. Wright
##' @references
##'   Janitza, S., Celik, E. & Boulesteix, A.-L., (2016). A computationally fast variable importance test for random forests for high-dimensional data. Adv Data Anal Classif \url{http://dx.doi.org/10.1007/s11634-016-0276-4}. \cr
...
importance_pvalues <- function(x, method = c("janitza", "altmann"), num.permutations = 100, formula = NULL, data = NULL, ...) {
  ...
  if (method == "janitza") {
    ...
    vimp <- sapply(1:num.permutations, function(i) {
      ...
      blockForest(formula, dat, num.trees = x$num.trees, mtry = x$mtry, min.node.size = x$min.node.size, 
